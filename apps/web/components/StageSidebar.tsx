...
export const StageSidebar: React.FC<StageSidebarProps> = ({
  ...
  return (
    <>
      {/* Mobile Horizontal Stage Bar (< lg screens) */}
      <div className="lg:hidden bg-defence-sidebar border-b border-defence-border p-3 overflow-x-auto">
        <div className="flex items-center space-x-2 min-w-max">
          {stages.map((stage) => {
            ...
            let style = 'bg-defence-card/40 text-defence-text/60 border-defence-border/40 opacity-60 cursor-not-allowed';
            if (isCompleted) {
              style = 'bg-defence-green/10 text-defence-green border-defence-green/40 hover:bg-defence-green/20';
            } else if (isActive) {
              style = 'bg-defence-cyan/20 text-defence-cyan border-defence-cyan font-bold hud-border-cyan';
            } else if (!isLocked) {
              style = 'bg-defence-card text-defence-heading border-defence-border hover:border-defence-border/80';
            }
            ...
            return (
              <button
                ...
                disabled={isLocked}
                className={`px-3 py-1.5 rounded-lg border text-xs font-mono flex items-center space-x-1.5 transition ${style}`}
              >
                <span>S{stage.stage_order}</span>
                <span className="truncate max-w-[100px]">{stage.name}</span>
                {isLocked ? (
                  <Lock className="w-3 h-3 text-slate-500" />
                ) : isCompleted ? (
                  <CheckCircle2 className="w-3 h-3 text-defence-green" />
                ) : null}
              </button>
            );
          })}
      ...
      <aside className="hidden lg:flex w-72 lg:w-80 bg-defence-sidebar border-r border-defence-border h-[calc(100vh-57px)] flex-col justify-between p-4 overflow-y-auto shrink-0">
        <div>
